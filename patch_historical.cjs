...
// We need to calculate highestHistoricalPercent right before the rendering of historicalPeriods
          ...
          <div className="bg-white p-6 rounded-[2rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)]">
             ...
             <div className="space-y-4">
                {stats.historicalPeriods.map((p, idx) => {
                   ...
                   return (
                   <div key={idx} className={\`border-b \${isHighest ? 'border-emerald-100 bg-emerald-50/30 p-3 -mx-3 rounded-xl' : 'border-gray-100'} last:border-0 pb-4 last:pb-0\`}>
                      <div className="flex justify-between items-center mb-2">
                         ...
                         <span className={\`text-xs font-bold px-2 py-1 rounded-md \${isHighest ? 'bg-shimmer-emerald shadow-sm border border-emerald-200' : p.brushedPercent >= 90 ? 'bg-emerald-50 text-emerald-700' : 'bg-gray-100 text-gray-600'}\`}>
...
file = file.replace(
  /\{\/\* Historical Comparison \*\/\}[\s\S]*?<\/div>\s*\}\)\}\s*<\/div>\s*<\/div>\s*\)/,
  replacementBlock
);
